...
- Understand the app shape: Spring Boot 3, layered (controller → service → repository), PostgreSQL, JWT-based auth, and a Google Gemini integration.
- Key files to read first: `src/main/java/com/example/demo/DemoApplication.java`, `security/JwtUtil.java`, `controller/UserController.java`, `service/RecommendationService.java`, `service/UserService.java`, `repository/*`, `entity/*`.
...
## Big picture (short)
- Runtime: Spring Boot application (`DemoApplication`). Controllers expose REST endpoints under `/api/*` (see `controller/UserController.java`).
- Business logic: `service/*` (e.g., `RecommendationService` calls external AI; `UserService` implements CRUD and workout association).
- Persistence: Spring Data JPA repositories in `repository/*` (e.g., `UserRepository`, `WorkoutRepository`). The DB is PostgreSQL configured in `application.properties`.
...
- Controllers & Services: `src/main/java/com/example/demo/controller/UserController.java`, `src/main/java/com/example/demo/service/UserService.java` (pay attention to method names used by controllers).
- Persistence & schema: `src/main/java/com/example/demo/entity/*`, `src/main/java/com/example/demo/repository/*`. Note native query in `WorkoutRepository.findUserHistory`.
